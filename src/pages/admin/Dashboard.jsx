...
export default function Dashboard() {
  ...
  const [novoTelefone, setNovoTelefone] = useState(""); // <-- Novo estado para o Telefone
  ...
  async function handleAddStudent(e) {
    ...
    try {
      ...
      const dadosDoAluno = { 
        nome: novoNome, 
        email: novoEmail, 
        telefone: novoTelefone, // <-- Salvando o telefone
        plano: novoPlano, 
        status: "Ativo" 
      };
        ...
        {activeTab === "geral" && (
          <div>
            <header className="mb-8 flex justify-between items-center">
              ...
              <button onClick={() => setIsAlunoModalOpen(true)} className="bg-orange-500 hover:bg-orange-600 text-white px-5 py-3 rounded-xl font-semibold shadow-lg shadow-orange-500/20 active:scale-95 transition-all">+ Novo Aluno</button>
            </header>
        ...
        {activeTab === "alunos" && (
          <div>
            <header className="mb-8 flex justify-between items-center">
              ...
              <button onClick={() => setIsAlunoModalOpen(true)} className="bg-orange-500 hover:bg-orange-600 text-white px-5 py-3 rounded-xl font-semibold shadow-lg shadow-orange-500/20 active:scale-95 transition-all">+ Adicionar Aluno</button>
            </header>
                      ...
                      {alunos.map(aluno => (
                        <tr key={aluno.id} className="border-b border-slate-800/30 text-slate-300 hover:bg-slate-800/20 transition-colors">
                          ...
                          <td className="py-4 px-4 text-right">
                            {/* Botão de WhatsApp */}
                            <a 
                              href={getWhatsAppLink(aluno)}
                              target="_blank" 
                              rel="noreferrer"
                              className="inline-block text-green-500 hover:bg-green-500/10 px-3 py-2 rounded-lg transition-colors mr-2 text-sm font-semibold"
                            >
                              WhatsApp
                            </a>
        ...
        {activeTab === "treinos" && (
          <div>
            <header className="mb-8 flex justify-between items-center">
              ...
              <button onClick={() => setIsTreinoModalOpen(true)} disabled={alunos.length === 0} className="bg-orange-500 hover:bg-orange-600 disabled:bg-slate-800 disabled:text-slate-500 text-white px-5 py-3 rounded-xl font-semibold shadow-lg shadow-orange-500/20 active:scale-95 transition-all">+ Criar Treino</button>
            </header>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl">
            ...
            <form onSubmit={handleAddStudent} className="space-y-4">
              ...
              {/* NOVO CAMPO: TELEFONE */}
              <div>
                <label className="block text-slate-400 text-sm font-medium mb-1">Telefone / WhatsApp (com DDD)</label>
                <input type="text" placeholder="Ex: 11999998888" value={novoTelefone} onChange={(e) => setNovoTelefone(e.target.value)} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white outline-none focus:border-orange-500" />
              </div>
              <div>
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 my-8 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleAddWorkout} className="space-y-4">
              <div>
                ...
                <select required value={alunoSelecionado} onChange={(e) => setAlunoSelecionado(e.target.value)} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white outline-none focus:border-orange-500">
              ...
              <div>
                ...
                <input type="text" required placeholder="Ex: Treino A - Peito e Tríceps" value={treinoNome} onChange={(e) => setTreinoNome(e.target.value)} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white outline-none focus:border-orange-500" />
              </div>
              <div className="border-t border-slate-800 pt-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold">Exercícios</h3>
                  <button type="button" onClick={handleAddExercicioField} className="text-sm text-orange-500 hover:text-orange-400 font-bold">+ Adicionar Exercício</button>
                </div>
                <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                  {exercicios.map((ex, index) => (
                    <div key={index} className="grid grid-cols-12 gap-2 bg-slate-950 p-3 rounded-xl border border-slate-800">
                      <div className="col-span-6"><input type="text" placeholder="Nome" required value={ex.nome} onChange={(e) => handleExercicioChange(index, "nome", e.target.value)} className="w-full bg-slate-900 border border-slate-800 px-3 py-2 rounded-lg text-sm text-white outline-none focus:border-orange-500" /></div>
                      <div className="col-span-2"><input type="number" placeholder="Séries" required value={ex.series} onChange={(e) => handleExercicioChange(index, "series", e.target.value)} className="w-full bg-slate-900 border border-slate-800 px-3 py-2 rounded-lg text-sm text-white text-center outline-none focus:border-orange-500" /></div>
                      <div className="col-span-2"><input type="text" placeholder="Reps" required value={ex.reps} onChange={(e) => handleExercicioChange(index, "reps", e.target.value)} className="w-full bg-slate-900 border border-slate-800 px-3 py-2 rounded-lg text-sm text-white text-center outline-none focus:border-orange-500" /></div>
                      <div className="col-span-2"><input type="number" placeholder="Kg" value={ex.carga} onChange={(e) => handleExercicioChange(index, "carga", e.target.value)} className="w-full bg-slate-900 border border-slate-800 px-3 py-2 rounded-lg text-sm text-white text-center outline-none focus:border-orange-500" /></div>
                    </div>
                  ))}
                </div>
              </div>
